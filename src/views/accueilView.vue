<template>
  <section id="hero">
    <div class="text-content">
      ...
      <strong>
        <h5>
          Des formations pratiques, adaptées au marché local, pour réussir dans la vente
          de produits et services
        </h5>
      </strong>
      <p>
        Formations sectorielles, bootcamps intensifs et coaching personnalisé pour booster
        vos ventes.
      </p>
      <button>
...
const visibleFormations = computed(() => {
  ...
  return Array.from(
    { length: visibleCount },
    (_, i) => formations.value[(start + i) % total]
  );
});

const nextFormation = () => {
  currentFormationIndex.value =
    (currentFormationIndex.value + 1) % formations.value.length;
};
...
const faqItems = ref([
  {
    ...
    answer:
      "Nos formations sont ouvertes à tous : vendeurs, entrepreneurs, commerçants, et toute personne souhaitant améliorer ses compétences en vente.",
    isOpen: false,
  },
  {
    question: "Quel est le format des formations (en présentiel ou en ligne) ?",
    answer:
      "Nous proposons des formations en présentiel dans nos centres et en ligne via notre plateforme e-learning.",
    isOpen: false,
  },
  {
    question: "Y a-t-il un suivi après la formation ?",
    answer:
      "Oui, un accompagnement post-formation est disponible sous forme de coaching et de sessions de mise en pratique.",
    isOpen: false,
  },
  {
    question: "Comment puis-je m'inscrire ?",
    answer:
      "Cliquez sur le bouton 'Inscrivez-vous Maintenant' et remplissez le formulaire en ligne. Vous serez contacté par un conseiller.",
    isOpen: false,
  },
]);
